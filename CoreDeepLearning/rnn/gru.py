...
from CoreDeepLearning.layers import MatrixWeight
from CoreDeepLearning.rnn.rnn import RNN
from CoreDeepLearning.shared import SharedMemory
from CoreDeepLearning.syntax import Var, Concat, Sigmoid, Wx, Tanh, Const, WxBiasLinear, Linear
...
class GRU(RNN):
    def __init__(self, seq_length, in_size, hidden_size, initialize, scale=1):
        ...
        self.Wz = MatrixWeight(x_and_h_size, hidden_size, initialize, scale)
        self.Wr = MatrixWeight(x_and_h_size, hidden_size, initialize, scale)
        self.W = MatrixWeight(x_and_h_size, hidden_size, initialize, scale)
        self.last_h = SharedMemory(np.zeros(hidden_size))

        self.decoderW = MatrixWeight(hidden_size, in_size, initialize, scale)
